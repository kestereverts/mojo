...
public abstract class PircBot implements ReplyConstants {
    ...
    private long _messageDelay = 2000;
    private long _burstLength = 10;
    private long _burstRecoveryTime = 5000;
    ...
    private final Hashtable<String, Hashtable<String, User>> _channels = new Hashtable<String, Hashtable<String, User>>();
    ...
    private boolean _multiThreadedInput = false;
    private final Map<String, Channel> _channelObjects;
    private final LinkedList<NetworkUser> networkUsers = new LinkedList<NetworkUser>();
     ...
     */
    public PircBot() {
        _channelObjects = Collections.synchronizedMap(new HashMap<String, Channel>());
    }
     ...
     */
    public final synchronized void connect(String hostname, boolean useSSL) throws IOException, IrcException, NickAlreadyInUseException {
        this.connect(hostname, 6667, null, useSSL);
    }
     ...
     * @throws NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname, int port, boolean useSSL) throws IOException, IrcException, NickAlreadyInUseException {
        this.connect(hostname, port, null, useSSL);
    }
     ...
     * @throws NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname, int port, String password, boolean useSSL) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if (useSSL) {
            try {
                ...
                Logger.getLogger(PircBot.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private Socket createSSLSocket(String hostname, int port) throws NoSuchAlgorithmException, KeyManagementException, IOException {
        ...
        return sc.getSocketFactory().createSocket(hostname, port);
    ...
    public final synchronized void reconnect() throws IOException, IrcException, NickAlreadyInUseException {
        ...
        connect(getServer(), getPort(), getPassword(), getUseSSL());
    }
    ...
    public void startTimers() {
        ...
        TimerTask ping = new TimerTask() {
            ...
            @Override
            public void run() {
                PircBot.this.sendRawLine("PING :ALIVE");
            }

        };

        timer.scheduleAtFixedRate(ping, 0, 25000);
    }
    ...
    protected void handleLine(String line) {
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            if(sourceNick.equalsIgnoreCase(getNick())) {
                ...
                synchronized(_channelObjects) {
                    if(!_channelObjects.containsKey(channel.toLowerCase())) {
                        _channelObjects.put(channel.toLowerCase(), new Channel(channel.toLowerCase()));
                    }
                }
            }
            NetworkUser networkUser = addNetworkUser(sourceLogin, sourceHostname, "", sourceNick, false, false, false, false, -1, "");
            Channel channelObj = _channelObjects.get(channel.toLowerCase());
            if(channelObj != null) {
                channelObj.addUser(networkUser, '\0');
            }
            ...
            synchronized(_channelObjects) {
                Channel channel = _channelObjects.get(target.toLowerCase());
                if(channel != null) {
                    NetworkUser networkUser = getNetworkUser(sourceNick);
                    channel.removeUser(networkUser);
                }
            }
            this.removeUser(target, sourceNick);
            if (sourceNick.equals(this.getNick())) {
                synchronized(_channelObjects) {
                    _channelObjects.remove(target.toLowerCase());
                }
                this.removeChannel(target.toLowerCase());
            }
            ...
            if (sourceNick.equals(this.getNick())) {
                ...
                NetworkUser networkUser = getNetworkUser(sourceNick);
                removeNetworkUserFromAllChannels(networkUser);
            ...
            if(channel != null) {
                synchronized(channel) {
                    ...
                    channel.setTopicSetBy(sourceNick);
                    channel.setTopicSetAt(new Date());
                }
            } else {
                System.out.println("CHANNEL NOT FOUND!!!!!");
            }
            this.onTopic(target, topic, sourceNick, System.currentTimeMillis(), true);
    ...
    private final void processServerResponse(int code, String response) {
        ...
        if (code == RPL_LIST) {
            ...
            if(channelObj != null) {
                ...
                System.out.println("CHANNEL IS NULL BY RPL_TOPIC");
            }
            ...
            if(channelObj != null) {
                synchronized(channelObj) {
                    channelObj.setTopicSetBy(setBy);
                    channelObj.setTopicSetAt(new Date(date));
                }
            }
            ...
            for(int i = 0; tokenizer.hasMoreTokens(); i++) {
                ...
                switch(i) {
                    case 0: // our nick
                        break;
                    case 1: // the channel
                        if(!_channelObjects.containsKey(token.toLowerCase())) {
                            return;
                        }
                        channel = token;
                        break;
                    ...
                    case 5: // nick
                        nick = token;
                        break;
                    case 6: // away/registered/isBot/isIrcOp/sign
                        away = token.contains("G");
                        registered = token.contains("r");
                        isBot = token.contains("B");
                        isIrcOp = token.contains("*") || token.contains("!");
                        char possibleSign = token.charAt(token.length() - 1);
                        if(!Character.isLetterOrDigit(possibleSign) && possibleSign != '*') {
                            sign = possibleSign;
                        }
                        break;
                    case 7: // hopCount
                        try {
                            hopCount = Integer.parseInt(token.substring(1));
                        }
                        catch (NumberFormatException e) {
                            hopCount = -1;
                        }
            ...
            NetworkUser networkUser = addNetworkUser(ident, host, server, nick, away, registered, isBot, isIrcOp, hopCount, realName);
            Channel channelObj = _channelObjects.get(channel.toLowerCase());
            if(channelObj == null) {
                System.out.println("CHANNEL IS NULL BY RPL_WHOREPLY");
                return;
            }
            channelObj.addUser(networkUser, sign);
    ...
    public final void setBurstLength(long burstLength) {
        if(burstLength < 0) {
            throw new IllegalArgumentException("burstLength cannot be negative");
        }

        _burstLength = burstLength;
    }

    public final long getBurstLength() {
        return _burstLength;
    }

    public final void setBurstRecoveryTime(long burstRecoveryTime) {
        if(burstRecoveryTime < 0) {
            throw new IllegalArgumentException("burstRecoveryTime cannot be negative");
        }

        _burstRecoveryTime = burstRecoveryTime;
    }

    public final long getBurstRecoveryTime() {
        return _burstRecoveryTime;
    }
    ...
    private final void removeChannel(String channel) {
        ...
        synchronized (_channels) {
            ...
            _channelObjects.remove(channel.toLowerCase());
        }
    ...
    public Channel getChannel(String channel) {
        return _channelObjects.get(channel.toLowerCase());
    }

    public NetworkUser addNetworkUser(String ident, String host, String server, String nick, boolean away,
            boolean registered, boolean isBot, boolean isIrcOp, int hopCount, String realName) {
        synchronized(networkUsers) {
            for(NetworkUser networkUser : networkUsers) {
                synchronized(networkUser) {
                    if(networkUser.getNick().toLowerCase().equals(nick.toLowerCase())) {
                        networkUser.setAll(ident, host, server, nick, away, registered, isBot, isIrcOp, hopCount, realName);
                        return networkUser;
                    }
                }
            }
            NetworkUser networkUser = new NetworkUser(ident, host, server, nick, away, registered, isBot, isIrcOp, hopCount, realName);
    ...
    public NetworkUser getNetworkUser(String nick) {
        for(NetworkUser networkUser : networkUsers) {
            if(networkUser.getNick().equalsIgnoreCase(nick)) {
                return networkUser;
            }
        }
        return null;
    ...
    public void removeNetworkUser(NetworkUser networkUser) {
        Iterator<NetworkUser> i = networkUsers.iterator();
        while(i.hasNext()) {
            if(i.next() == networkUser) {
                i.remove();
                return;
            }
        }
    ...
    public void removeNetworkUserFromAllChannels(NetworkUser networkUser) {
        Collection<Channel> channels = _channelObjects.values();
    ...
    public boolean isMultiThreadedInput() {
        return _multiThreadedInput;
    }

    public void setMultiThreadedInput(boolean _multiThreadedInput) {
        this._multiThreadedInput = _multiThreadedInput;
    }
}
